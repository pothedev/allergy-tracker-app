...
start_shift_model = RandomForestRegressor(n_estimators=100, random_state=42)
end_shift_model = RandomForestRegressor(n_estimators=100, random_state=42)
...
# Visualization function for Random Forest
def visualize_random_forest(model, feature_names, tree_index=0):
    """
    Visualizes a single decision tree from a Random Forest model.
    :param model: The RandomForestRegressor model
    :param feature_names: List of feature names
    :param tree_index: Index of the tree to visualize
    """
    single_tree = model.estimators_[tree_index]  # Extract the tree
    ...
    graph.render(f"random_forest_tree_{tree_index}", format="png", cleanup=True)
    graph.view()

# Example usage: Visualize the first tree of the start_shift_model
feature_names = ['temp (°C)', 'humidity (%)', 'clouds (%)', 'rain (mm)']
visualize_random_forest(start_shift_model, feature_names)
